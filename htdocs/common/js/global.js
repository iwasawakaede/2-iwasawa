...
// ↑上の式だと下の分岐点にはどちらも当てはまらない×ので、結果は何も起こらない・・・
		...
		// console.log("8");
		$('.js-headerTop').addClass('is-upMove').parent('.l-header').addClass('background');//.js-headerTopに.is-upMove.opacityのクラス名を追加(上に上がる動き)
// console.log("9");
    }else {
		// 4（小さい）→（下にスクロールすると、）ヘッダーが上から出現する
		$('.js-headerTop').removeClass('is-upMove');	//.js-headerTopにis-upMoveというクラス名を除き
		// console.log("6");
		$('.js-headerTop').addClass('is-downMove').parent('.l-header').removeClass('background');//.js-headerTopに.is-downMove.opacityのクラス名を追加
		// console.log("7");
    }
